...
RUN pip install --upgrade pip && pip install autopep8 && pip install Keras && pip install tensorflow 

# コンテナ側のルート直下にworkという名前の作業ディレクトリを作り移動する
WORKDIR /work
...
# "--notebook-dir=/work" => Jupyter Labのルートとなるディレクトリを指定
